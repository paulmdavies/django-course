...
def home(request):
    recent_post_titles = {slug: posts_dict[slug]['title'] for slug in list(posts_dict.keys())[-3:]}

    return render(
        request,
        'blog/home.html',
        {
            'post_titles': recent_post_titles
        }
    )

def posts(request):
    all_post_titles = {slug: posts_dict[slug]['title'] for slug in list(posts_dict.keys())}

    return render(
        request,
        'blog/posts.html',
        {
            'post_titles': all_post_titles
        }
    )
